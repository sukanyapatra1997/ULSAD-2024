...
def get_configurable_parameters_custom(
    ...
    if config.dataset.format.lower() in ("btech", "mvtec", "visa", "mvtec_loco", "mpdd"):
        project_path = project_path / config.dataset.category

    # add category subfolder if needed
    if config.dataset.format.lower() in ("cifar10", "fmnist", "mnist"):
        project_path = project_path / ('noise_' + str(config.dataset.contamination).split('.')[1])
