...
def make_mpdd_dataset(
    ...
    """Create MPDD samples by parsing the MPDD AD data file structure.
    ...
    Args:
        ...
        create_validation_set (bool, optional): Boolean to create a validation set from the test set.
            MPDD AD dataset does not contain a validation set. Those wanting to create a validation set
            could set this flag to ``True``.

    Examples:
        The following example shows how to get training samples from MPDD AD bottle category:

        >>> root = Path('./MPDD')
        >>> category = 'bottle'
        >>> path = root / category
        >>> path
        PosixPath('MPDD/bottle')

        >>> samples = make_mpdd_dataset(path, split='train', split_ratio=0.1, seed=0)
    Returns:
...
class MPDDDataset(AnomalibDataset):
    ...
    def _setup(self) -> None:
        self.samples = make_mpdd_dataset(self.root_category, split=self.split, extensions=IMG_EXTENSIONS)
...
class MPDD(AnomalibDataModule):
    ...
    Args:
        root (Path | str): Path to the root of the dataset
        category (str): Category of the MPDD dataset (e.g. "tubes" or "metal_plate").
